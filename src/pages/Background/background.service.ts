...
import apiService, { ProfileVideo } from "@services/ApiService";
...
const fetchVideoItems = async ({secUid, nick}) => {
  let lastCursor = getLastCursor(nick);
  let response = {hasMore: true, itemList: [] as ProfileVideo[]};
  ...
  while (response.hasMore) {
    ...
    storageService.saveProfileVideos(nick, response.itemList)

    chrome.runtime.sendMessage({
      action: "onStorage",
      data: {storageKey: `videoItems_${nick}`, videoItems: response.itemList}
    });

    lastCursor = getLastCursor(nick);
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  switch (request.action) {
    case "start-analyze":
      ...
      break;
    case "fetchVideoItems":
      fetchVideoItems(request.data)
      break;
  }
});
